...
def hyper_pa(degree_distribution, edgesize_distribution, max_edgesize, nodes):
    ...
    cum_sum = np.zeros(max_edgesize+1, dtype=np.int64)
    ...
    for n in range(edges[-1][-1]+1,nodes+1):
        for _ in range(degree_distribution.sample()):
            ...
            if new_edgesize > 1:
                binom = 1
                acc = 0
                for source_edgesize in range(new_edgesize-1, max_edgesize+1):
                ...
                if total == 0:
                    ...
                    source_edgesize = bisect_left(cum_sum[new_edgesize-1:], key)+new_edgesize-1
