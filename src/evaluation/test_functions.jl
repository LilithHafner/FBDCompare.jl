...
function old_hyperpa(size::Integer=45000; name = "DAWN")
    ...
    nodes > 200 && @warn "old_hyperpa expected runtime on the order of $(round(Integer, (nodes/200)^2)) minute(s)\n"
    ...
    try
        ...
        x isa ProcessFailedException || rethrow()
        return [[1]]
    end
    isfile(output_file) ? [parse.(Int, split(line)) for line in readlines(output_file)] : [[1]]
end
